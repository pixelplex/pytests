...
class EthereumTransactions(object):
    ...
    def get_transfer_transaction(self, web3, to, value, _from=None, nonce=None, value_currency="ether", gas=2000000,
                                 gas_price=10, gas_price_currency="gwei", signer=ETH_PRIVATE_KEY, debug_mode=False):
        transfer_props = deepcopy(self.get_operation_json("transfer_operation"))
        if _from is not None:
            transfer_props.update({"from": _from})
        if nonce is None:
            nonce = web3.eth.getBlock(web3.eth.defaultBlock).get("number")
        transfer_props.update({"nonce": nonce, "to": to, "value": web3.toWei(value, value_currency), "gas": gas,
                               "gasPrice": web3.toWei(gas_price, gas_price_currency)})
        if debug_mode:
            lcc.log_debug("Ethereum transfer operation: \n{}".format(json.dumps(transfer_props, indent=4)))
        return [transfer_props, signer]

    @staticmethod
    def broadcast(web3, transaction, debug_mode=False, log_transaction=False, log_transaction_logs=False):
        if debug_mode:
