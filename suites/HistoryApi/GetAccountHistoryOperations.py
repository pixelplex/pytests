...
class GetAccountHistoryOperations(BaseTest):
    ...
    def method_main_check(self):
        ...
        params = [self.account, operation_id, start, stop, limit]
...
class PositiveTesting(BaseTest):
    ...
    def add_balance_to_new_account(self, account, operation_count, transfer_amount):
        ...
        amount = (operation_count * transfer_amount) + (operation_count * fee)
    ...
    def call_get_account_history_operations(self, account, operation_id, start, stop, limit):
    ...
    def new_account_history(self, get_random_valid_account_name):
        ...
        params = [new_account, operation_id, start, stop, limit]
        response_id = self.send_request(self.get_request("get_account_history_operations", params),
                                        self.__history_api_identifier)
        response = self.get_response(response_id)
    ...
    def operation_id_to_retrieve(self, get_random_valid_account_name, get_random_valid_asset_name):
        ...
        new_account = self.get_account_id(new_account, self.__database_api_identifier,
                                          self.__registration_api_identifier)
        operation = self.echo_operations.get_operation_json("asset_create_operation", example=True)
        fee = self.get_required_fee(operation, self.__database_api_identifier)[0]["amount"]
        operation = self.echo_operations.get_transfer_operation(echo=self.echo, from_account_id=self.account_1,
                                                                to_account_id=new_account, amount=fee)
        collected_operation = self.collect_operations(operation, self.__database_api_identifier)
        broadcast_result = self.echo_operations.broadcast(echo=self.echo, list_operations=collected_operation,
                                                          log_broadcast=False)
        ...
        if self.is_operation_completed(broadcast_result, expected_static_variant=0):
            operation_count = 1
            response = self.call_get_account_history_operations(new_account, transfer_operation_id, start, stop, limit)
            check_that(
                "'number of history results'",
        ...
        operation = self.echo_operations.get_asset_create_operation(echo=self.echo, issuer=new_account,
        ...
        if self.is_operation_completed(broadcast_result, expected_static_variant=1):
            ...
            limit = 1
            response = self.call_get_account_history_operations(new_account, create_asset_operation_id, start, stop,
                                                                limit)
            check_that(
                "'number of history results'",
    ...
    def limit_operations_to_retrieve(self, get_random_valid_account_name):
        ...
        response = self.call_get_account_history_operations(new_account, operation_id, start, stop, max_limit)
        check_that(
            "'number of history results'",
            # todo: remove '+ 1' ("transfer_operation: faucet") to run on an empty node.
            len(response["result"]), is_(operation_count + 1)
        ...
        response = self.call_get_account_history_operations(new_account, operation_id, start, stop, min_limit)
        check_that(
            "'number of history results'",
        ...
        max_limit = 100
        self.fill_account_history_with_transfer_operations(new_account, self.account_1, max_limit - operation_count - 1)
    ...
    def stop_and_start_operations(self, get_random_integer, get_random_integer_up_to_hundred):
        ...
        response = self.call_get_account_history_operations(self.account_1, operation_identifier, start, stop, limit)
